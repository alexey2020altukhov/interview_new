...
### Основные отличия версий Java
Java 8 принесла функциональный стиль программирования: Lambda, Stream API, Optional, новую Date/Time API. 
Java 11 в основном улучшила стандартную библиотеку, добавила новый HttpClient и убрала Java EE модули из JDK. 
Java 17 добавила records (удобные DTO), sealed classes (ограничение наследования), pattern matching для instanceof и стала одной из основных LTS-версий для enterprise. 
Java 21 принесла virtual threads, которые упрощают написание высоконагруженных приложений с большим количеством параллельных операций.

### Jenkins это
Jenkins — это инструмент автоматизации CI/CD. Он позволяет создавать pipeline, которые автоматически собирают проект, запускают тесты, создают артефакты или Docker-образы и выполняют деплой. Pipeline обычно описывается через Jenkinsfile и состоит из этапов build, test и deploy.

### Горизонтальное и вертикальное масштабирование
Масштабируемость (scalability) — это способность системы справляться с ростом нагрузки.

Вертикальное масштабирование (Scale Up) - увеличение мощности одного сервера.

Горизонтальное масштабирование (Scale Out) - добавление новых экземпляров приложения.

### Jwt токен из чего состоит?
JWT (JSON Web Token) используется в основном для аутентификации и передачи информации о пользователе между клиентом и сервером.

WT состоит из Header, Payload и Signature. Header содержит алгоритм подписи, Payload — claims пользователя и служебные данные, Signature позволяет проверить целостность токена. JWT не шифрует данные, а только подписывает их, поэтому чувствительные данные в Payload хранить нельзя.

### Кубер
Kubernetes используется для управления контейнеризированными приложениями. Основные сущности — Pod, Deployment, Service, Ingress, ConfigMap и Secret. Deployment отвечает за количество экземпляров приложения и обновления, Service предоставляет стабильный доступ к Pod, а Kubernetes автоматически перезапускает упавшие контейнеры и позволяет масштабировать приложение.

CI/CD — это процесс автоматизации сборки, тестирования и доставки приложения. Обычно после push в Git запускается pipeline: сборка проекта, запуск тестов, создание Docker image, загрузка его в registry и деплой в Kubernetes.
...
### Порядок выполнения команд БД:
Логически SQL выполняется начиная с FROM, затем JOIN, WHERE, GROUP BY, HAVING, SELECT, DISTINCT, ORDER BY и в конце LIMIT/OFFSET. 
